...
import static cdc.BinlogOffset.BINLOG_FILENAME_OFFSET_KEY;
 ...
 *
 * debezium mysql connector task 需要的,有状态的上下文
 * 当发送数据时候,需要把offset change and schema change 记录下来
 * 通过 MySqlSplit??
 *
 * RecordEmitter   MySqlRecordEmitter
 *
 * 该类准备了 MySqlStreamingChangeEventSource需要的所有东西,将结果数据通过
 * Queue 暴露出来
 */
public class StatefulTaskContext {
    ...
    private MySqlOffsetContext loadStartingOffsetState(
        ...
        BinlogOffset offset = mySqlSplit.getStartingOffset();
