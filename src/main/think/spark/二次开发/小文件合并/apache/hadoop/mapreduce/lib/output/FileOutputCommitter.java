...
public class FileOutputCommitter extends PathOutputCommitter {
  ...
  // XX has O(1) recursive delete, so this parameter is left false by default.
  ...
  public void getAllFile(FileStatus file, List<FileSize> allFiles,FileSystem fs,Path path) throws IOException {
      if(file.isFile()){
        allFiles.add(new FileSize(file.getPath().getParent().toString(),file.getPath().getName(),file.getLen(),path.toString()));
      }else{
        if(file.isDirectory()){
          FileStatus[] fileStatuses = fs.listStatus(file.getPath());
          for(FileStatus one : fileStatuses){
            getAllFile(one,allFiles,fs,path);
          }
        }
      }
  }
  ...
  protected void commitJobInternal(JobContext context) throws IOException {
    if (hasOutputPath()) {
      ...
      //获取所有已提交的文件 如果不是1 这里可以获取到吗?
      ArrayList<FileSize> fileSizes = new ArrayList<FileSize>();
      for (FileStatus stat: getAllCommittedTaskPaths(context)) {
          getAllFile(stat,fileSizes,fs,stat.getPath());
      }
      ...
      Map<String, List<FileSize>> collect = fileSizes.stream().collect(
              Collectors.groupingBy(
                      FileSize::processRelaPath, Collectors.toList()));

      collect.forEach((path,list)->{
        //把所有的合并成一个文件
        MergeXXParquetFile.setFileSystem(fs);
        try {
          MergeXXParquetFile.mergeParquet(new Path(finalOutput,path.substring(1)).toString());
        } catch (Exception e) {
          e.printStackTrace();
        }
      });
